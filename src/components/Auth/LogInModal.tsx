...
const LogInModal = ({setLogIn}: any) => {
  ...
  return (
    <div className="fixed w-screen h-screen text-xl flex justify-center z-50">
      ...
      <form
        className="absolute flex flex-col bg-white w-10/12 mx-auto h-5/6 my-[5%] p-4 pt-24 gap-16 max-w-4xl items-center rounded-3xl overflow-y-scroll"
        onSubmit={e => e.preventDefault()}
      >
        <input
          className="w-full sm:w-1/2 input input-bordered min-h-12"
          type="text"
        ...
        />
        <button
          className="w-full sm:w-1/2 btn btn-outline btn-primary"
          onClick={() => tryLogIn(username, password)}
        >
          Se connecter
        </button>
        <div
          className={`w-full sm:w-1/2 alert alert-error ${
            errorMessage === '' ? 'hidden' : 'block'
          } shadow-lg flex flex-row`}
        >
          <svg
